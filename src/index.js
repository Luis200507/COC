...
const API_KEY1 = "AIzaSyAG5F-JedmaCNP2e1mXjisdM0yCCcYFr-4"; //principal
const API_KEY2 = "AIzaSyCO6SJoqXTjMjYvPZM06CJ9_5TFgk72KKE"; //ssq
const API_KEY3 = "AIzaSyAdihOCr2jukTcf_CHgEnpBBzzsq5DExy0"; // jm89
const API_KEY4 = "AIzaSyCYSCDz0ehlHejTKT6z5OqLfzLFHTmUUV8"; // amominhamajuzinha, file manager key
const regex = /\d+/g;
...
async function runPrompts() {
  ...
  moduloInicial = await inquirer.number({
...
const fileManager = new GoogleAIFileManager(API_KEY4);
const genAI4 = new GoogleGenerativeAI(API_KEY4)
const model4 = genAI4.getGenerativeModel({ model: "gemini-1.5-flash", safetySettings });
const imageChat = model4.startChat({ generationConfig });
//#region 
const genAI1 = new GoogleGenerativeAI(API_KEY1);
const genAI2 = new GoogleGenerativeAI(API_KEY2);
const genAI3 = new GoogleGenerativeAI(API_KEY3);
const model1 = genAI1.getGenerativeModel({ model: "gemini-1.5-flash", safetySettings });
const model2 = genAI2.getGenerativeModel({ model: "gemini-1.5-flash", safetySettings });
const model3 = genAI3.getGenerativeModel({ model: "gemini-1.5-flash", safetySettings });
const chat1 = model1.startChat({ generationConfig });
const chat2 = model2.startChat({ generationConfig });
const chat3 = model3.startChat({ generationConfig });
...
function getNextChatSession() {
  const chatSession = chats[chatIndex];
  chatIndex = (chatIndex + 1) % chats.length; // Rotaciona o índice
  return chatSession;
}
...
async function verificarImagens(page, tipo, conteudoDaQuestao, alternativas) {
  ...
  for (let i = 0; i < allImages.length; i++) {
    ...
    if (i < allImages.length - 1) {
      const respostaIntermediaria = await imageChat.sendMessageStream([
        {
          fileData: {
      ...
      if (tipo === 'Objetiva') {
        promptQuestion = `Interprete as imagens repassadas e resolva a questão. Me retorne somente a letra correta, quero APENAS um caractere na sua resposta, sem me explicar.${mathConcate}: alternativas (em json, interprete-as.) = ${alternativas}, conteudo = ${conteudoDaQuestao.texto}`
      ...
      respostaFinal = await imageChat.sendMessageStream([
        {
          fileData: {
            mimeType: getResult.mimeType,
            fileUri: getResult.uri
...
async function responderQuestao(page) {
  let chatSession = getNextChatSession();
  // console.log('Respondendo questão atual com API_KEY, final: ...' + (chatSession._apiKey).slice(-4))
  try {
  ...
  if (typeOfQuestion === 'Objetiva') {
    ...
    let alternativasString = ''
    alternativas.forEach(option => {
      alternativasString += `${option.letra}: ${option.texto}; `
    })
    ...
    while (tentativa < maxTentativas) {
      ...
      if (tentativa === 0) {
        if (conteudoDaQuestao.containsImage) {
          resposta = await verificarImagens(page, typeOfQuestion, conteudoDaQuestao, alternativasString);
        } else {
          prompt = `resolva a seguinte questao, faça a sua resolução em segundo plano, leve o tempo que precisar para resolver. Entretanto, após isso me retorne somente a letra da alternativa correta, sem mais nada, quero apenas um caractere na resposta: ${conteudoDaQuestao.texto}, ${alternativasString}`;
          resposta = await chatSession.sendMessageStream(prompt)
        }
      } else {
        prompt = `resposta errada, por favor tente novamente, procure o possivel erro que vc cometeu, e tente outra alternativa diferente das já tentadas, e lembre-se: traga somente a letra da alternativa e nada mais, sem explicar. ${conteudoDaQuestao.texto}, ${alternativasString}`;
        resposta = await chatSession.sendMessageStream(prompt)
      }
...
async function responderExers(page) {
  for (let atividadeAtual = 1; atividadeAtual <= numeroDeAtividades; atividadeAtual++) {
    ...
    for (let questao of questoes) {
      ...
      while (!success && tentativas < maxTentativas) {
        try {
          ...
          console.error(chalk.bgRed(`Falha ao responder a questão ${questao.numero}:`), err);
          await asleep(15);
    ...
    let nota = '';
    let scoreText = '';
    const score = await page.$('.exerciselist-card-score');

    if(score) {
      scoreText = (await page.evaluate(el => el.innerText , score)).replace(/[^0-9,]/g, '')
    ...
    } else if (incorrectInThisExer > 0) {
      nota = 'Uma ou mais questões nao foram resolvidas, nota indefinida'
    } else {
    ...
    if(atividadeAtual !== numeroDeAtividades) await Promise.all([
...
(async () => {
  ...
  await page.type(".hkKGjB", await credenciais.email);
  await page.type(".hVUGKM", await credenciais.password);
  await Promise.all([
    page.waitForNavigation(),
    page.click(".cHpGAw")
  ]);
